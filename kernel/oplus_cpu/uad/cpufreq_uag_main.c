...
void set_soft_limit_freq(struct cpufreq_policy *policy, unsigned int soft_freq)
{
	...
	cpu_val_systrace_c(soft_freq, first_cpu, "frameboost_limit");
...
{
	struct uag_gov_policy *sg_policy = sg_cpu->sg_policy;
	bool is_migration = sg_cpu->flags & WALT_CPUFREQ_IC_MIGRATION_BIT;
	bool is_hiload;
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST)
	int mid_stune_boost, max_stune_boost;
#endif
	...
	if (is_hiload && !is_migration)
		max_and_reason(util, sg_policy->hispeed_util, sg_cpu, CPUFREQ_REASON_HISPEED_BIT);

	if (is_hiload && nl >= mult_frac(cpu_util, NL_RATIO, 100))
		max_and_reason(util, *max, sg_cpu, CPUFREQ_REASON_NWD_BIT);
	...
	if (sg_policy->flags & SCHED_CPUFREQ_EARLY_DET) {
		...
		if (ed_task_boost_type == ED_TASK_BOOST_MID)
			...
			cpu_util = cpu_util < max_stune_boost ? max_stune_boost : cpu_util;
		max_and_reason(util, cpu_util, sg_cpu, CPUFREQ_REASON_EARLY_DET_BIT);
	}
#endif
}
#endif /* CONFIG_ARCH_MEDIATEK */
...
#endif
		if (unlikely(eas_opt_debug_enable))
			trace_printk("[eas_opt]: cluster_id: %d, capacity: %lu, util_thresh: %d, util_orig: %lu, "
...
#endif
		if (unlikely(eas_opt_debug_enable))
			trace_printk("[eas_opt]: cluster_id: %d, util_orig: %lu, util: %lu, avg_nr_running: %d, oplus_cap_multiple: %d,nr_oplus_cap_multiple: %d, util_thresh: %d\n",
...
static int __init cpufreq_uag_init(void)
{
	...
	register_uag_hooks();
#if IS_ENABLED(CONFIG_OPLUS_SCHED_TUNE)
	_stune_util = (stune_util_t)(*_kallsyms_lookup_name)("stune_util");
#endif
